...
- CI uses one **`macos`** job on `macos-latest`. `svg3-render` rasterises with wgpu and its tests need a real GPU adapter, which the `ubuntu-latest` runner lacks — so lint, tests, coverage, `app-macos`, and CodSpeed benchmark steps run together on macOS (Metal):
  - `cargo fmt --check`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`, `cargo llvm-cov --workspace --exclude app-macos --all-features --lcov --output-path lcov.info` (runs the tests under coverage instrumentation — the headless `svg3-render` render test exercises Metal here), `codecov/codecov-action` upload, `cargo test -p app-macos`, `cargo codspeed build --workspace --exclude app-macos`, and `CodSpeedHQ/action` in `mode: simulation`.
