//! Pipeline + bind-group-layout construction for the shape, filter, and
//! image draw paths.
//!
//! All of these are stateless device builders called once from
//! [`Renderer::with_device`](super::renderer::Renderer::with_device) — they
//! don't touch any runtime state, so they live outside the [`Renderer`]
//! impl block to keep the renderer itself focused on the encode path.

use crate::render::mesh::VERTEX_ATTRIBUTES;
use crate::render::Vertex;

use super::clear::DEPTH_FORMAT;
...
/// interpolation precision); [`build_render_plan`](crate::render::scene)
/// instead hands each 2D shape a small per-shape forward Z bias so painter's
...
pub(super) fn build_filter_bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
...
pub(super) fn build_image_bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
...
pub(super) fn build_filter_pipeline(
    device: &wgpu::Device,
    format: wgpu::TextureFormat,
...
/// [SPEC.md](../../../../SPEC.md) §7.3).
pub(super) fn target_filter_depth_stencil() -> wgpu::DepthStencilState {
...
/// filter's spatial Z.
pub(super) fn build_composite_bind_group_layout(device: &wgpu::Device) -> wgpu::BindGroupLayout {
    device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
...
pub(super) fn build_image_pipeline(
    device: &wgpu::Device,
    format: wgpu::TextureFormat,
    bind_group_layout: &wgpu::BindGroupLayout,
) -> wgpu::RenderPipeline {
...
pub(super) fn premultiplied_alpha_blend() -> wgpu::BlendState {
    wgpu::BlendState {
