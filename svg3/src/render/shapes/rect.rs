//! SVG 1.1 `<rect>` — geometry resolution plus fill and stroke tessellation.
//!
...
/// Convert the rectangle outline to a Lyon path for stroke and marker logic.
pub(crate) fn to_path(geo: &RectGeometry) -> Path {
    let mut builder = Path::builder().with_svg();
    if geo.rx == 0.0 || geo.ry == 0.0 {
        builder.move_to(point(geo.x, geo.y));
        builder.line_to(point(geo.x + geo.width, geo.y));
        builder.line_to(point(geo.x + geo.width, geo.y + geo.height));
        builder.line_to(point(geo.x, geo.y + geo.height));
        builder.close();
        return builder.build();
    }

    let right = geo.x + geo.width;
    let bottom = geo.y + geo.height;
    ...
    builder.move_to(point(geo.x + geo.rx, geo.y));
    builder.line_to(point(right - geo.rx, geo.y));
    builder.arc_to(radii, rotation, flags, point(right, geo.y + geo.ry));
    builder.line_to(point(right, bottom - geo.ry));
    builder.arc_to(radii, rotation, flags, point(right - geo.rx, bottom));
    builder.line_to(point(geo.x + geo.rx, bottom));
    builder.arc_to(radii, rotation, flags, point(geo.x, bottom - geo.ry));
    builder.line_to(point(geo.x, geo.y + geo.ry));
    builder.arc_to(radii, rotation, flags, point(geo.x + geo.rx, geo.y));
    builder.close();
    builder.build()
}
