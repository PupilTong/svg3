...
//! parsing and [`render`]'s `<rect>`/`<circle>`/`<ellipse>` path are
//! implemented, but [`style`] is still a skeleton, so `render_str`
//! currently surfaces the
...
/// render `<rect>`/`<circle>`/`<ellipse>` geometry today, use
